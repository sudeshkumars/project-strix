  ...
  async execute (client, interaction) {
    ...
    if (sub === 'add') {
      ...
      if (existing.length >= 20) {
        return interaction.editReply(errorCard('Limit reached', ['You can have at most 20 highlights.']))
      }
      if (existing.find(h => h.word === word)) {
        return interaction.editReply(errorCard('Already added', [`\`${word}\` is already in your highlights.`]))
      }

      db.addHighlight(guildId, userId, word)
      return interaction.editReply(successCard('Highlight Added', [`You'll be notified when \`${word}\` is mentioned.`]))
    }

    if (sub === 'remove') {
      const word = interaction.options.getString('word').toLowerCase()
      db.removeHighlight(guildId, userId, word)
      return interaction.editReply(successCard('Removed', [`\`${word}\` removed from highlights.`]))
    }

    if (sub === 'list') {
      const highlights = db.getUserHighlights(guildId, userId)
      if (!highlights.length) return interaction.editReply(infoCard('\u{1f514} Your Highlights', ['You have no highlight words.']))

      return interaction.editReply(infoCard('\u{1f514} Your Highlights', [highlights.map(h => `\`${h.word}\``).join(', ')]))
    }

    if (sub === 'clear') {
      const highlights = db.getUserHighlights(guildId, userId)
      for (const h of highlights) db.removeHighlight(guildId, userId, h.word)
      return interaction.editReply(successCard('Cleared', ['All highlights removed.']))
    }
  }
}
